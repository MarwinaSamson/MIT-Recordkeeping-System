...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Submitted Applications (Privacy Consent): {privacy_consents}\n')
        ...
        if non_superusers > 0:
            ...
            for idx, student in enumerate(students, 1):
                self.stdout.write(f'{idx}. {student.username} ({student.email})')
                
                # Check personal details
                try:
                    personal = PersonalDetails.objects.get(user=student)
                    self.stdout.write(f'   ✓ Personal Details: {personal.first_name} {personal.last_name}')
                ...
                try:
                    ...
                    self.stdout.write(f'   ✓ Application: {app.application_id} ({app.status})')
        ...
        if applications == 0 and personal_details > 0:
            ...
            self.stdout.write('Students have filled forms but no Application records exist.')
            self.stdout.write('You need to:')
            self.stdout.write('  1. Run migrations: python manage.py migrate')
            self.stdout.write('  2. Create Application records for existing students')
            self.stdout.write('  3. Or run: python manage.py initialize_admin_data\n')
        elif applications > 0:
            self.stdout.write(self.style.SUCCESS('\n✓ All set! Application records exist in database.'))
            self.stdout.write('You can access the admin dashboard without running initialize_admin_data.\n')
