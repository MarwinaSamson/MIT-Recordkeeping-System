...
const saved = JSON.parse(localStorage.getItem("educationalBackground") || "{}");
const container = document.getElementById("sections");

LEVELS.forEach((level) => {
  const d = saved[level.key] || {};
  const reqLabel = level.required ? '<span class="text-red-500">*</span>' : "";
  ...
  container.innerHTML += `
        <div class="border border-gray-200 rounded-xl p-5 space-y-4 mb-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input type="text" name="${level.key}_schoolName" value="${
              d.schoolName || ""
            ...
            <p class="text-xs text-red-500 mt-1 hidden" data-error="${
              level.key
            }_schoolName"></p>
            </div>
            <div>
              <label class="block text-sm font-semibold mb-1.5">Degree/Course ${reqLabel}</label>
            <input type="text" name="${level.key}_degree" value="${
              d.degree || ""
            }" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand/40 focus:border-brand transition" />
            <p class="text-xs text-red-500 mt-1 hidden" data-error="${
              level.key
            ...
            <input type="number" name="${
              level.key
            }_yearCompleted" min="1950" max="2030" value="${
              d.yearCompleted || ""
            }" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand/40 focus:border-brand transition" />
            <p class="text-xs text-red-500 mt-1 hidden" data-error="${
              level.key
...
document.getElementById("eduForm").addEventListener("submit", function (e) {
  ...
  // Get scholarship value (outside the loop)
  const scholarship = this.elements["scholarship"]
  ...
  if (valid) {
    ...
    let hiddenInput = this.querySelector('input[name="levels_data"]');
    if (!hiddenInput) {
      hiddenInput = document.createElement("input");
      hiddenInput.type = "hidden";
      hiddenInput.name = "levels_data";
    ...
    // Also save to localStorage for potential use in other steps
