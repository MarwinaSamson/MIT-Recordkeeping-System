            ...
            fileInput.addEventListener('change', function () {
                if (this.files && this.files[0]) {
                    ...
                    if (file.type.startsWith('image/')) {
                        ...
                        previewElement.innerHTML = '<i class="fas fa-file-pdf text-2xl text-red-500"></i>';
                    } else {
                        previewElement.innerHTML = '<i class="fas fa-file text-2xl text-gray-400"></i>';
                    }
                } else {
        ...
        setupPreview(document.querySelector('.deansRec-file'), document.querySelector('.deansRec-preview'));
        setupPreview(document.querySelector('.honorable-file'), document.querySelector('.honorable-preview'));
        setupPreview(document.querySelector('.gsat-file'), document.querySelector('.gsat-preview'));
        setupPreview(document.querySelector('.tor-pdf-file'), document.querySelector('.tor-pdf-preview'));
        setupPreview(document.querySelector('.psa-pdf-file'), document.querySelector('.psa-pdf-preview'));

        // ===== TOR Image Section =====
        function setupImageSection(containerId, addBtnId, itemClass, inputClass, previewClass, removeBtnClass, nameAttr) {
        ...
        setupImageSection('tor-images-container', 'add-tor-image', 'tor-image-item', 'tor-image-input', 'tor-image-preview', 'remove-tor-image', 'torImages[]');
        setupImageSection('psa-images-container', 'add-psa-image', 'psa-image-item', 'psa-image-input', 'psa-image-preview', 'remove-psa-image', 'psaImages[]');
        ...
        document.getElementById("documentsForm").addEventListener("submit", function (e) {
            ...
            if (!torHasPDF && !torHasImage) {
                showError("tor", "Please upload either a PDF file or at least one image of your TOR");
                valid = false;
            }
            ...
            if (!psaHasPDF && !psaHasImage) {
                showError("psa", "Please upload either a PDF file or at least one image of your PSA");
                valid = false;
            }
            ...
            const torImageCount = Array.from(torImages).filter(input => input.files.length > 0).length;
            const psaImageCount = Array.from(psaImages).filter(input => input.files.length > 0).length;
