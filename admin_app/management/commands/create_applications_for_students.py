...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write('Creating Application records for existing students...\n')
        
        # Get all non-superuser students
        students = User.objects.filter(is_superuser=False)
        
        if not students.exists():
            self.stdout.write(self.style.WARNING('No student users found in database.'))
            return
        ...
        for student in students:
            ...
            if Application.objects.filter(user=student).exists():
                skipped_count += 1
                self.stdout.write(f'⊘ {student.email}: Application already exists')
                continue
            ...
            created_count += 1
            self.stdout.write(self.style.SUCCESS(f'✓ {student.email}: Created {application_id} ({status})'))
        
        self.stdout.write(f'\n{self.style.SUCCESS("Created: " + str(created_count))}')
        self.stdout.write(f'{self.style.WARNING("Skipped: " + str(skipped_count))}')
        
        if created_count > 0:
            self.stdout.write(self.style.SUCCESS('\n✓ Application records created successfully!'))
            self.stdout.write('You can now access the admin dashboard to view and manage applications.')
